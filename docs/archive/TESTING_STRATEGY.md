...
### Graphify v1 coverage

- Retrieval-before-read: `noesis_attend(graphQueries)` must query Graphify before file reads in graph-backed turns.
- Stale penalty: stale INFERRED graph evidence is penalized at belief-commit time, with the 0.55 floor preserved.
- Freshness metadata: stale-hours values propagate into the preamble instead of a fixed constant.
- Parse resilience: malformed Graphify JSON emits a warning and still returns an empty findings list.
- Provenance: graph findings keep query/timestamp context through attention and preamble rendering.
...
| `confidence-strategy.ts` | Graphify mapping | 5 |
